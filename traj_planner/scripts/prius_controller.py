...
from velocity_planner import VELOCITY_PLANNER
inf = 1000000000000
rospy.init_node('PRIUS_CONTROLLER',anonymous = True)

class CONTROLLER:
    def __init__(self, x_waypoints, y_waypoints, vel_setpoints, yaw_setpoints):
        ...
        self.max_speed = 50 # KM/H
        self.target_speed = self.max_speed / 3.6 # M/S
        self.min_speed = 20 / 3.6
        self.a = 1
        self.x_trajectory = x_waypoints
        self.y_trajectory = y_waypoints
        # self.x_waypoints, self.y_waypoints = x_waypoints, y_waypoints
        self.yaw_trajectory = yaw_setpoints
        # self.z_waypoints = z_waypoints
        # self.slope()
        # self.get_trajectory()
        # self.v_to_be_followed = np.clip(vel_setpoints, self.min_speed, self.target_speed)
...
#**************************************************************************************
    ...
    def start(self):
        ...
        while (self.last_traj_index - target_index > 4) :
            # print("hello hello")
        ...
        # for i in range(10):
        #     self.control_publish(a_i, 0, 0,'FORWARD')
        self.slow_down(0)
...
if __name__ == '__main__':
    ...
    if yf ==1:
        ...
        print("enter a valid lane:: defaulting to lane 1")
        yf=3
    print("enter final velocity in the lane")
    xvf=input()/3.6
    vel_plan = VELOCITY_PLANNER(xf,yf,xvf)
    time.sleep(2)
    # print(np.shape(vel_plan.velocity_setpoints),np.shape(vel_plan.yaw_setpoints))
    control = CONTROLLER(vel_plan.x_setpoints, vel_plan.y_setpoints,vel_plan.velocity_setpoints,vel_plan.yaw_setpoints)
    time.sleep(0.2)
    control.start()